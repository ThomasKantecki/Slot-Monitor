"""Cardiology-only public-flow enumerator for Orlando Health MyChart.

Place beside epic_scheduling_extractor_OH.py. The script enumerates Cardiology, every supported Cardiology visit type, every directly schedulable
reason, and every finite answer branch exposed by the anonymous decision tree. It does not book, hold, authenticate, or submit patient-identifying data.
Terminal and ineligible branches are traversed only far enough for MyChart to
return their public stop result, which is written to the audit.
"""
...
OUTPUT_ROOT = "OH_cardiology_all_public_flows_outputs"
ACCEPTED_BASELINE_AUDIT = (
    Path(__file__).resolve().parent / OUTPUT_ROOT / "13sep_OH_cardio_all_flows_150249"
    / "13sep_OH_cardio_all_flows_150249_flow_audit.json"
)
...
def run_name() -> str:
    return time.strftime("%d%b").lower() + "_OH_cardio_all_flows_" + time.strftime("%H%M%S")
...
def submit_answer(client: epic.OrlandoHealthApiSession, response: dict[str, Any], answer: str) -> dict[str, Any]:
    ...
    payload = {
        ...
        "question": epic.core_question_answer(question, answer),
    }
    fields = epic.postify(payload)
    fields.append(("__RequestVerificationToken", client.csrf_token))
    return client.post_json("decision_tree", fields, {
        "phase": "enumerator_tree_answer", "question_prompt": question.get("Prompt", ""), "answer": answer,
    })
...
def save(run_dir: Path, rows, audit, captures):
    prefix = run_dir.name
    rows = list({(r.get("flow_id"), r.get("provider_id"), r.get("department_id"),
                  r.get("display_datetime_utc"), r.get("appointment_date"), r.get("appointment_time")): r
    ...
    (run_dir / f"{prefix}_slots.json").write_text(json.dumps(rows, indent=2, ensure_ascii=False), encoding="utf-8")
    (run_dir / f"{prefix}_flow_audit.json").write_text(json.dumps(audit, indent=2, ensure_ascii=False), encoding="utf-8")
    (run_dir / f"{prefix}_network_capture.json").write_text(json.dumps(captures, indent=2, ensure_ascii=False), encoding="utf-8")
    print(f"PASS: {len(rows)} unique slots; {len(audit)} audit rows", flush=True)


def run(args):
    epic.SITE_CODE = "OH_CARD_ALL_FLOWS"
    ...
    try:
        ...
        specialties = [
            ...
            if norm(specialty.get("Name")) == "cardiology"
        ]
        if not specialties:
            raise RuntimeError("Cardiology was not found in the anonymous public specialty catalog")
        ...
        failure_statuses = {
            ...
            "page_guard_reached", "repeated_page_stop", "repeated_continue_stop", "excluded",
        }
        incomplete_flows = [item for item in audit if item.get("status") in failure_statuses]
        if args.only_flow_id:
            health_status = "diagnostic_incomplete" if incomplete_flows else "diagnostic_complete"
        else:
            health_status = "incomplete" if incomplete_flows else "accepted"
        health = {
...
def main():
    ...
    parser.add_argument("--api-delay", type=float, default=0.50)
    ...
    parser.add_argument("--baseline-flow-audit", default=str(ACCEPTED_BASELINE_AUDIT),
