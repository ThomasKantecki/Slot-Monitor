...
| `npm run audit:layout` | `audit-layout.cjs` | Opens every built page at twenty widths (the same as zooming the browser) and two heights and checks that no two elements overlap, nothing runs past the screen edge, no text is clipped and every dialog fits. Same Chrome and `PLAYWRIGHT_CORE` requirement as `audit:pages`. |
