...
export function motionRasterTransform(current, snapshot, frame) {
  const scale = current.k / snapshot.k;
  const padShift = snapshot.pad * (1 - scale);
  const currentX = frame.ox + frame.s * current.x;
  const currentY = frame.oy + frame.s * current.y;
  const snapshotX = frame.ox + frame.s * snapshot.x;
  const snapshotY = frame.oy + frame.s * snapshot.y;
  return {
    scale,
    x: currentX - scale * snapshotX + padShift,
