...
function drawRaster(){if(!RASTER_OK)return;ensureRasterPaths();
 const dpr=2;raster.width=Math.round(W*dpr);raster.height=Math.round(H*dpr);
...
function beginRasterMotion(){svg.classList.add("zooming");if(!rasterReady||Z.k>RASTER_ZOOM_LIMIT)return;syncMapFrame();rasterActive=true;raster.style.opacity="1";vp.style.visibility="hidden";applyRasterZoom();}
...
function applyZoom(){if(rasterActive&&Z.k>RASTER_ZOOM_LIMIT)endRasterMotion();if(rasterActive)applyRasterZoom();else vp.setAttribute("transform","translate("+Z.x+" "+Z.y+") scale("+Z.k+")");}
