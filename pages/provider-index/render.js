...
.control-section{flex:0 1 auto;min-width:max-content;margin:0;padding:5px 7px 7px;border:2px solid #9aa8b7;background:#f8fafb}
...
.geography-controls{flex:1 1 300px}.geography-controls .control-section-body{flex-wrap:nowrap}.geography-controls .search-stack{flex:1 1 120px;min-width:96px}.geography-controls .fgroup,.geography-controls input.control{width:100%;max-width:none;min-width:96px}
...
@container (max-width:699px){.comparison-controls,.location-controls,.subspecialty-controls{flex:1 0 auto}}
...
.subspecialty-controls{flex:0 1 214px;min-width:180px}.subspecialty-controls[hidden]{display:none}.subspecialty-controls select.control{width:100%;min-width:0;max-width:none;text-overflow:ellipsis}
...
.pill-help>.filter-pill[aria-pressed="true"]{background:var(--accent)}.pill-help:has(>.filter-pill[aria-pressed="true"]){background:var(--accent);border-color:var(--accent)}
...
/* the boxes wrap onto a second row whenever they do not fit; none of them shrinks below its own controls */
...
@media (min-width:881px) and (max-height:680px){
 ...
 .control-section{padding:4px 6px 6px}.control-section-body{gap:5px}
...
<div class="stage">
 <section class="panel mapbox">
  ...
  <div class="controls">
   ...
   <fieldset class="control-section subspecialty-controls" hidden><legend>Sub-specialty</legend><div class="control-section-body"><select id="spec" class="control" aria-label="Sub-specialty"></select></div></fieldset>
...
document.querySelector(".subspecialty-controls").hidden=subLabels.length===0;
...
function refreshAreaOptions(){const input=document.getElementById("area-search"),list=document.getElementById("area-options");input.value="";input.placeholder=gran==="zip"?"ZIP code":"County";list.innerHTML=L().paths.map(p=>'<option value="'+esc(p.k)+'"></option>').join("");}
function setGran(g){if(g===gran)return;gran=g;document.getElementById("g-county").setAttribute("aria-pressed",String(g==="county"));document.getElementById("g-zip").setAttribute("aria-pressed",String(g==="zip"));refreshAreaOptions();switchLayer();}
document.getElementById("g-county").onclick=()=>setGran("county");document.getElementById("g-zip").onclick=()=>setGran("zip");
document.getElementById("area-search").onchange=(e)=>{const key=e.target.value.trim();if(L().paths.some(p=>p.k===key))showProviders(key);};
...
if(SOLO){
 ...
 const comparisonControls=document.getElementById("comparison-controls");if(comparisonControls)comparisonControls.remove();
 const div=document.querySelector(".tdiv"); if(div)div.remove();
 document.title=SYS[SOLO]+" Provider Index";
}
